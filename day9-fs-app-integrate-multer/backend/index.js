...
const app = express();
app.use(cors()); // Für den Fehler: Access to fetch at ... from origin ... has been blocked by CORS policy: ...
...
app.get("/api/v1/transactions/:id", (req, res) => {
  ...
  readTransactions()
    .then(
      (transactions) =>
        transactions.find((t) => t.id.toString() === transactionId) // {...transaction...} | undefined
    )
    .then((foundTransaction) => res.status(200).json(foundTransaction || {})) // wenn die foundTransaction undefined ist, dann ein leeres Objekt senden
